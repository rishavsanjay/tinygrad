...
class TestJit(unittest.TestCase):
  def test_graph_rebinds_copy_views(self):
    ...
    dst, src = UOp.param(0, dtypes.uint8, 16, "CPU"), UOp.param(1, dtypes.uint8, 16, "CPU:1")
    ...
    inputs = tuple(UOp.new_buffer(d, 16, dtypes.uint8) for d in ("CPU", "CPU:1"))
    graph = GraphRunner(create_graph_call([call]).src[0], inputs)
    inputs = tuple(UOp.new_buffer(d, 16, dtypes.uint8) for d in ("CPU", "CPU:1"))
    bufs = dict(graph.updated_buffers(0, inputs))
    self.assertEqual((bufs[0].offset, bufs[1].offset), (8, 4))
    self.assertEqual((bufs[0].base, bufs[1].base), tuple(x.buffer for x in inputs))
