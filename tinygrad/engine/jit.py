...
class GraphRunner:
  def __init__(self, linear:UOp, input_uops:tuple[UOp, ...]=()):
    ...
    for call in self.linear.src:
      # Bulk STORE arguments can be views of inputs: replay must preserve their offsets when rebinding the backing buffers.
      buffer_replace = [(p, b) for p, b in enumerate(get_call_arg_uops(call))
                        if b.op is Ops.PARAM or (call.body.op is Ops.STORE and any(x.op is Ops.PARAM for x in b.toposort()))]
