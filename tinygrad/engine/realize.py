...
def exec_hcq(ctx:ExecContext, call:UOp, ast:UOp) -> list[float|None]:
  if (info:=call.arg.aux).inputs:
    ...
    for i, alignment, name in info.input_alignments:
      if addrs[i] % alignment: raise ValueError(f"unaligned {name} address {addrs[i]:#x}; expected {alignment}-byte alignment")
