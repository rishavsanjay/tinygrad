...
def run_linear(linear:UOp, var_vals:dict[str, int]|None=None, input_uops:Sequence[UOp]=(), update_stats=True, jit=False, wait=False):
  ...
  if not jit:
    linear = link_linear(compile_linear(linear, validate=VALIDATE_WITH_CPU, input_uops=inputs), cache=bool(input_uops), input_uops=inputs)
    if not getenv("DISABLE_PRE_INSTANTIATE"):
      for call in linear.src:
        if call.op is Ops.CALL and len(call.src) > 1 and call.src[0].op is Ops.PROGRAM:
          for d in to_tuple(call.src[1].device):
            get_runtime(d, call.src[0], cache=True)
