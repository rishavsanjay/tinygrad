...
from tinygrad.helpers import resolve_pool_pads, round_up, IMAGE, IMAGE_DOT, FLOAT16, WINO, getenv
...
class OpMixin(ElementwiseMixin, ReduceMixin):
  ...
  def dot(self, w:Self, dtype:DTypeLike|None=None) -> Self:
    ...
    ```
    """
    if IMAGE and IMAGE_DOT: return self.image_dot(w, dtype)
  ...
  def image_conv2d(self, weight:Self, bias:Self|None=None, groups=1, stride=1, dilation=1, padding=0, dtype=None) -> Self:
    ...
    def ipad(t, i, amt):
      pad_value = 0 if getenv("QCOM_IMAGE_ZERO_PAD", 0) else Invalid
      return t.pad(tuple(None if d != i else (0, amt-s) for d,s in enumerate(t.shape)), value=pad_value) if amt != t.shape[i] else t
