...
class MetalProgram(Program[MetalDevice]):
  ...
  def __call__(self, *bufs, global_size:tuple[int,int,int]=(1,1,1), local_size:tuple[int,int,int]=(1,1,1), vals:tuple[int, ...]=(), wait=False, **kw):
    ...
    for i,(a,arg) in enumerate(zip(TinyELF.merge_args(self.signature, bufs, vals), self.signature)):
      if arg.addrspace is AddrSpace.ALU:
        encoder.setBytes_length_atIndex(bytes(getattr(ctypes, f"c_int{arg.dtype.bitsize}")(a)), arg.dtype.itemsize, i)
