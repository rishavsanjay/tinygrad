...
class QCOMPerfCounters:
  """Bracket GPU work between two command-stream counter snapshots and report deltas.

  Usage: perf.begin(); run_work(); deltas = perf.end()
  Counters are free-running, so deltas attribute the window to the bracketed work.
  The read submissions run on the same KGSL context as the workload, whose
  submissions execute in order, so no extra cross-queue waits are needed.
  """
  def __init__(self, dev:QCOMDevice):
    ...
    # CP_REG_TO_MEM_0_64B snapshots a full 64B block per read (Turnip's counter path), so each
    # counter needs a 64-byte destination slot; the LO/HI pair lands in its first two dwords.
    self.buf = dev._gpu_alloc(self.slot_vals * 64 * 2, uncached=True, fill_zeroes=True)
    self.results:dict[str, int]|None = None
    self._last_end_ao:int|None = None
  ...
  def _read_slot(self, slot:int):
    dev = self.dev()
    assert dev is not None, "device collected while profiling"
    q = cast("Callable[[], QCOMComputeQueue]", dev.hw_compute_queue_t)()
    # CP_SCOPE_CNTL(DISABLE_PREEMPTION) keeps foreign-context work out of the window, but while the
    # scope is armed no later IB exists to re-enable it, so a host wait on the newest submission
    # (e.g. dev.synchronize() inside the window) never sees its retirement and the context wedges.
    # It is therefore opt-in and only safe with the begin/end submissions bracketing all synced work.
    scope = getenv("QCOM_PERF_SCOPE", 0)
    if scope and slot == 0: q.cmd(mesa.CP_SCOPE_CNTL, 1)
    self.emit_snapshot(q, int(self.buf.va_addr) + slot * self.slot_vals * 64)
    if scope and slot == 1: q.cmd(mesa.CP_SCOPE_CNTL, 0)
    q.submit(dev)

  @staticmethod
  def gpu_clock() -> int:
    with contextlib.suppress(OSError): return int(open(f"{KGSL_SYSFS}/gpuclk").read())
    return 0

  def begin(self):
    self.results, self.gpu_clk_begin = None, self.gpu_clock()
    self._read_slot(0)
  ...
  def end(self) -> dict[str, int]:
    ...
    # The read IBs do not signal the timeline, so dev.synchronize() can return before the GPU has
    # executed them (especially from a low-power idle). ALWAYS_ON is a free-running 19.2 MHz clock
    # that is never zero, so poll its slot until the end read lands on the uncached buffer.
    deltas, self._last_end_ao = self.fetch_record(self.buf.cpu_view(), 0, self._last_end_ao)
    self.results = deltas
    self.results["GPU_CLK_KHZ"] = self.gpu_clock() or self.gpu_clk_begin
    return self.results
