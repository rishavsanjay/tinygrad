...
HCQ_DEVS = frozenset(("CPU",)) | (frozenset(("AMD",)) if HCQ2 else frozenset())
# NOTE (a8xx-max): QCOM is intentionally absent. Our A8xx backend runs on the HCQ1 runtime
# (per-signal KGSL retirement waits, PM4 counter snapshots, image descriptors), which the hcq2
# batch/encode path cannot express yet. Leaving QCOM in this set routes its calls through
# lower_call, which requires per-device pm_encode rules that only exist for hcq2 backends.
# Revisit when the QCOM hcq2 port lands; until then QCOM uses the legacy exec_kernel path
# (including the proven HCQGraph capture used by TinyJit).
