...
def __getattr__(nm):
  ...
  srcs="https://gitlab.freedesktop.org/mesa/mesa/-/archive/mesa-26.2.1/mesa-26.2.1.tar.gz",
  # Mesa adds this C-only deprecation pragma to register macros.
  rules=[(r'\b__FD_DEPRECATED\b', ''), (r'_Pragma\s*\("GCC warning \\"Deprecated reg builder\\""\)', '')],
  dll=f"'tinymesa_cpu' if DEV.renderer == 'LVP' else 'tinymesa', {tinymesa_path}, emsg='pip install tinymesa==26.2.1'",
