...
class NVComputeQueue(NVQueue):
  ...
  def exec(self, call:UOp, prg:UOp):
    ...
    kargs = [vals[arg.slot].ccast(dtypes.uint64 if data.mock else arg.dtype) if arg.addrspace is AddrSpace.ALU else bufs[arg.slot].getaddr(self.devs)
             for arg in data.signature]
    qmd.patches |= dict(layout_args(kargs, at))
...
class NVProgramData:
  def __init__(self, dev:NVDevice, obj:TinyELF):
    ...
    # Arguments follow driver params in rendered ABI order. MockGPU treats each entry as a 64-bit generic argument.
    self.signature, self.mock = signature, mock
    if mock: self.cbuf_0[80:82] = [len(signature), 0]
